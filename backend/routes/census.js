...
const CENSUS_CACHE_TTL_MS = 24 * 60 * 60 * 1000;
const CENSUS_STATE_DELAY_MS = 300;
...
function readCache(cache, key) {
    ...
    if (Date.now() - entry.cachedAt > CENSUS_CACHE_TTL_MS) {
        cache.delete(key);
        return null;
    }
    return entry.data;
...
async function fetchCensusUrl(url, retries = 4) {
...
async function getCachedCensusData(cacheKey, cache, fetchFn) {
...
function handleCensusRouteError(res, error, label) {
    const status = error.response?.status;
    console.error(`[crime] ${label} failed:`, error.message);
    if (status === 429) {
        return res.status(429).json({
            success: false,
            message: "Census API rate limit reached. Wait a minute and try again, or use cached production data.",
        });
    }
    return res.status(500).json({
        success: false,
        message: `Failed to fetch ${label} from Census API`,
    });
}
